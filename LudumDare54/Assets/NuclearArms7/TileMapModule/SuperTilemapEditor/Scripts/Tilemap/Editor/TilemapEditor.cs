    ...
    public partial class TilemapEditor : Editor
    {
        ...
        private void DoMapSceneGUI()
        {
            if (m_toggleMapBoundsEdit)
            {
                ...
                var fmh_1021_178_638315250672389125 = Quaternion.identity; Vector3 vMinX = Handles.FreeMoveHandle(m_tilemap.transform.TransformPoint(new Vector2(m_tilemap.MinGridX * m_tilemap.CellSize.x, m_tilemap.MapBounds.center.y)), 0.1f * HandleUtility.GetHandleSize(m_tilemap.transform.position), Vector3.zero, EditorCompatibilityUtils.CubeCap);
                var fmh_1022_185_638315250672415948 = Quaternion.identity; Vector3 vMaxX = Handles.FreeMoveHandle(m_tilemap.transform.TransformPoint(new Vector2((m_tilemap.MaxGridX + 1f) * m_tilemap.CellSize.x, m_tilemap.MapBounds.center.y)), 0.1f * HandleUtility.GetHandleSize(m_tilemap.transform.position), Vector3.zero, EditorCompatibilityUtils.CubeCap);
                var fmh_1023_178_638315250672420054 = Quaternion.identity; Vector3 vMinY = Handles.FreeMoveHandle(m_tilemap.transform.TransformPoint(new Vector2(m_tilemap.MapBounds.center.x, m_tilemap.MinGridY * m_tilemap.CellSize.y)), 0.1f * HandleUtility.GetHandleSize(m_tilemap.transform.position), Vector3.zero, EditorCompatibilityUtils.CubeCap);
                var fmh_1024_185_638315250672424710 = Quaternion.identity; Vector3 vMaxY = Handles.FreeMoveHandle(m_tilemap.transform.TransformPoint(new Vector2(m_tilemap.MapBounds.center.x, (m_tilemap.MaxGridY + 1f) * m_tilemap.CellSize.y)), 0.1f * HandleUtility.GetHandleSize(m_tilemap.transform.position), Vector3.zero, EditorCompatibilityUtils.CubeCap);
            ...
            if (EditorWindow.mouseOverWindow == SceneView.currentDrawingSceneView)
            {
                ...
                if (tile != null)
                {
                    ...
                    for (int i = 0; i < handlesPos.Count; ++i)
                    {
                        ...
                        {
                            var fmh_1119_75_638315250672431855 = Quaternion.identity; handlePos = Handles.FreeMoveHandle(handlePos, 0.1f * HandleUtility.GetHandleSize(m_tilemap.transform.position), Vector3.zero, EditorCompatibilityUtils.CubeCap);
